...
(function () {
  ...
  if (pEl) {
    ...
    var key = "mechvault-progress-" + (pEl.getAttribute("data-progress-key") || location.pathname.split("/").pop() || "home");
  ...
  function ensureSearchDrawer() {
    if (searchDrawer) return searchDrawer;
    ...
    searchDrawer.addEventListener("click", function (e) {
      if (e.target === searchDrawer || e.target.classList.contains("search-drawer-close")) closeSearchDrawer();
    });
    document.addEventListener("keydown", function (e) { if (e.key === "Escape") closeSearchDrawer(); });
    // Wire up the drawer's input
    var drawerInput = searchDrawer.querySelector(".search-input");
    var drawerResults = searchDrawer.querySelector(".search-results");
    drawerInput.addEventListener("input", function () { runSearch(this.value, drawerResults); });
    drawerInput.addEventListener("focus", function () { if (this.value) runSearch(this.value, drawerResults); });
    return searchDrawer;
  }
  ...
  // Mobile search trigger (icon in nav)
  document.addEventListener("click", function (e) {
    var t = e.target.closest(".search-trigger");
    if (t) openSearchDrawer();
  });
  ...
  var SITE = {
    semesters: {
      sem3: {
        ...
        label: "3rd Semester",
        subjects: [
           { code: "fm", name: "Fluid Mechanics", file: "/fm",
           ...
           { code: "som", name: "Strength of Materials", file: "/som",
           ...
           { code: "thermo", name: "Thermodynamics", file: "/thermo",
  ...
  /* ============================================================
     ...
     the old client-side password guard has been removed.
     ============================================================ */
})();